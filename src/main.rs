...
fn main() -> io::Result<()> {
    ...
    if let Some(path) = cli.tty_path {
        // If a path is provided, we enter "Normal Mode".
        println!("Normal Mode: Would take over TTY at {}", path);
    } else {
        // If no path is provided, we enter "Test Mode".
        run_tui_app()?;
    }
...
fn run_tui_app() -> io::Result<()> {
    ...
    loop {
        terminal.draw(|frame| {
            ...
            let login_form_rect: Rect = centered_rect(15, frame_area);
...
/// Helper function to create a centered rectangle of a given size inside a larger area.
